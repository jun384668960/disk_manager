...
UINT parse_exp_codes(U8* pBuffer, int* value,U8* bit_pos, U8 type)
{
...
int parse_scaling_list(U8* pBuffer,UINT sizeofScalingList,U8* bit_pos)
{
...
ERR get_h264_info(unsigned char* pBuffer,int size,AM_VIDEO_INFO* pH264Info)
{
	...
	BOOL residual_colour_tramsform_flag;
	...
	if (vui_parameters_present_flag) 
	{
		...
		if (timing_info_present_flag) 
		{
			...
			{
				U8 divisor; //when pic_struct_present_flag == 1 && pic_struct == 0
...
ERR UpdateVideoIdx(int deltaPts, UINT sampleSize,UINT chunk_offset, U8 sync_point)
{
	int i;
...
inline ERR UpdateAudioIdx(UINT sampleSize,UINT chunk_offset)
{
...
int put_VideoInfo(U8* pData, UINT size, UINT framerate, AM_VIDEO_INFO* h264_info)
{
	UINT nal_unit_length;
	U8 nal_unit_type;
	UINT cursor = 0;
	...
	do
	{
		...
		{ 
			if (_sps == NULL) 
			{
				...
				_sps = (U8*)malloc(_sps_size*sizeof(U8));
...
ERR put_VideoData(U8* pData, UINT size,PTS pts)
{
	UINT nal_unit_length;
	U8 nal_unit_type;
	UINT cursor = 0;
	UINT chunk_offset = mCurPos;
	U8 sync_point = 0;
	int ret = 0;
	do
	{
			...
			if (_pps == NULL) 
		  	{
				...
				_pps = (U8*)malloc(_pps_size*sizeof(U8));
 	...
 	int delta_pts = ((mVideoCnt == 0) ? 0 : (pts - _last_video_pts)*90000/1000);	  //一帧的时长
 	_last_video_pts = pts;
...
ERR put_AudioData(U8* pData,UINT dataSize,U32 frameCount)
{
#if 1
	U8 *pFrameStart;
	U32 frameSize;
	...
	while (GetOneFrame (&pFrameStart, &frameSize)) 
	{
		...
		UINT  header_length = sizeof(AdtsHeader);
		/*
...
void put_VideoTrackBox(UINT TrackId, UINT Duration)
{
	//TrackBox
...
void put_VideoMediaBox(UINT Duration)
{
	//MediaBox
	...
	put_buffer((U8 *)VIDEO_HANDLER_NAME, VIDEO_HANDLER_NAME_LEN-1);
...
void put_VideoMediaInformationBox()
{
	...
	U8 EncoderName[32]="\012AVC Coding"; //Compressorname
	...
	put_buffer((U8 *)_v_stsz, mVideoCnt * sizeof(_v_stsz[0]));
	...
	put_buffer((U8 *)_v_stco, mVideoCnt * sizeof(_v_stco[0]));
	...
	put_buffer((U8*)_stss, mStssCnt*sizeof(_stss[0]));
}
...
void put_AudioTrackBox(UINT TrackId, UINT Duration)
{
	//TrackBox
...
void put_AudioMediaBox()
{
	...
	put_buffer((U8 *)AUDIO_HANDLER_NAME, AUDIO_HANDLER_NAME_LEN-1);
...
void put_AudioMediaInformationBox()
{
	...
	put_be16((UINT)_audio_spec_config);
	put_be16(0x0000);
	put_byte(0x00);
	...
	put_be32(0);                                 //uint32 sampleSize
	put_be32(mAudioCnt);                         //uint32 sample_count
	put_buffer((U8 *)_a_stsz, mAudioCnt*sizeof(_a_stsz[0]));
	...
	put_buffer((U8 *)_a_stco, mAudioCnt * sizeof(_a_stco[0]));
}

void FindAdts (ADTS *adts, U8 *bs)
{
	UINT i = 0;
...
void FeedStreamData(U8 *inputBuf,U32 inputBufSize,U32 frameCount)
{
...
BOOL GetOneFrame(U8 **ppFrameStart,U32 *pFrameSize)
{
