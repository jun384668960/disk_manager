...
#define GOKE_AUDIODURATION_SYNCHRONIZATION   1 //国科的muxers方式
...
enum {max_adts = 8};
    ...
    ERR put_AudioData(U8* pData, UINT size, U32 frameCount);
    ERR FinishProcess();
#endif

    ERR get_time(UINT* time_since1904, char * time_str,  int len);
    ERR put_byte(UINT data);
    ERR put_be16(UINT data);
    ERR put_be24(UINT data);
    ERR put_be32(UINT data);
    ERR put_be64(U64 data);
    ERR put_buffer(U8 *pdata, UINT size);
    ERR put_boxtype(const char* pdata);
    UINT get_byte();
    void get_buffer(U8 *pdata, UINT size);
    void debug_sei();

		UINT read_bit(U8* pBuffer,  int* value,U8* bit_pos, UINT num);
    //UINT read_bit(U8* pBuffer,  int* value,U8* bit_pos = NULL, UINT num = 1);
    ...
    ERR UpdateVideoIdx(int pts, UINT sample_size,
                          UINT chunk_offset, U8 sync_point);
    ERR UpdateAudioIdx(UINT sample_size,
                          UINT chunk_offset);
    ...
    void FeedStreamData(U8* inputBuf,
                                U32 inputBufSize,
                                U32 frameCount);
    BOOL GetOneFrame(U8** ppFrameStart,
                                U32* pFrameSize);
